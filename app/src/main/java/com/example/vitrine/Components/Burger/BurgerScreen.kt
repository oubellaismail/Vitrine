...
import com.example.vitrine.Data.ProductData.burger
...
@Composable
fun BurgerScreen(navController: NavController, mainActivity: MainActivity) {
    val user: User? = mainActivity.user
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Burger Screen") },
                actions = {
                    IconButton(
                        onClick = {
                            if (user != null) {
    ...
    )
    {
        LazyColumn(
            ...
            items(burger) { category ->
                BurgerItem(category = category, navController = navController, mainActivity)
            }
        }
    }
}
