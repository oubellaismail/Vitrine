...
import com.example.vitrine.Data.ProductData.glace
...
@Composable
fun GlaceScreen(navController: NavController, mainActivity: MainActivity) {
    val user: User? = mainActivity.user
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Glace Screen") },
                actions = {
                    IconButton(
                        onClick = {
                            if (user != null) {
    ...
    )
    {
        LazyColumn(
            ...
            items(glace) { category ->
                GlaceItem(category = category, navController = navController, mainActivity)
            }
        }
    }
